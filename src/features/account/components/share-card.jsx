...
const INVITE_URL = "https://foxtrade.app/register";
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)] lg:gap-6">
      <GlowCard variant="warm">
        <CardBody className="flex flex-col gap-6 sm:flex-row sm:items-center">
          <div className="self-center rounded-xl bg-white p-3 sm:self-auto">
            <QRCodeSVG value={INVITE_URL} size={140} bgColor="#ffffff" fgColor="#0a0a0a" />
          </div>
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-5 flex items-center gap-2 rounded-lg border border-white/10 bg-field p-1 pl-3">
              <span className="min-w-0 flex-1 truncate text-sm text-neutral-300">{INVITE_URL}</span>
      ...
      <GlowCard as="section" aria-labelledby="invited-title">
        ...
        <SignInPrompt title="Sign in to track invites" text="Your personal invite code and the friends who joined appear here." />
      </GlowCard>
    </div>
  );
};
