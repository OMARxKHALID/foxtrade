...
export const placeTimedOrder = async (input) => {
  ...
  return asTrader("Sign in to place timed trades.", (user) => openTimed(user.id, parsed.data));
};

export const placePerpetualOrder = async (input) => {
  const parsed = perpetualOrderSchema.safeParse(input);
  if (!parsed.success) return validationFailure(parsed.error);
  return asTrader("Sign in to open positions.", (user) => openPosition(user.id, parsed.data));
};

export const closePosition = async (id) => {
  const parsed = positionIdSchema.safeParse(id);
  if (!parsed.success) return validationFailure(parsed.error);
  return asTrader("Sign in to manage positions.", (user) => close(user.id, parsed.data));
};

export const closeAllPositions = async () => asTrader("Sign in to manage positions.", (user) => closeAll(user.id));

export const cancelOrder = async (id) => {
  const parsed = positionIdSchema.safeParse(id);
  if (!parsed.success) return validationFailure(parsed.error);
  return asTrader("Sign in to manage orders.", (user) => cancelPendingOrder(user.id, parsed.data));
};

export const addMargin = async (input) => {
  const parsed = addMarginSchema.safeParse(input);
  if (!parsed.success) return validationFailure(parsed.error);
  return asTrader("Sign in to manage positions.", (user) => addPositionMargin(user.id, parsed.data.id, parsed.data.amount));
};
